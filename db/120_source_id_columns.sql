-- 120: doppio id per il source-sync — source_id dedicato, external_id intoccato.
--
-- BUG (produzione, 2026-09-07/08): il source-sync (docs/SOURCE_SYNC_PLAN.md)
-- scriveva l'id ESATTO della risorsa sul CRM sorgente stile CRM sorgente
...
-- colonna `source_id` (VARCHAR, testo libero, default '' = mai sincronizzato)
...
-- essere collegato a CRM sorgente A POSTERIORI (record già esistenti con un
...
--
-- contacts/opportunities hanno già `source_id` (migrazione 108): ADD COLUMN
...
BEGIN
  FOREACH t IN ARRAY tables LOOP
    EXECUTE format(
      'ALTER TABLE %I ADD COLUMN IF NOT EXISTS source_id VARCHAR(255) NOT NULL DEFAULT ''''', t
    );
    EXECUTE format(
      'CREATE UNIQUE INDEX IF NOT EXISTS idx_%I_source_id ON %I(source_id) WHERE source_id <> ''''', t, t
    );
  END LOOP;
END $$;
