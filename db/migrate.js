...
async function migrate() {
  await withMigrationLock(async () => {
    ...
    `);
    // Allineamento nomi file: alcuni .sql sono stati rinominati (naming interno
    // unificato su `source_*`). I database creati prima della rinomina hanno in
    // schema_migrations i vecchi nomi file: li riallineiamo ai nuovi per non
    // rieseguire per errore migration già applicate. Vedi db/131_*.sql.
    const RENAMED_MIGRATIONS = {
      "106_ghl_push_config.sql": "106_source_push_config.sql",
      "108_ghl_id_column.sql": "108_source_id_column.sql",
      "120_ghl_id_columns.sql": "120_source_id_columns.sql",
      "126_ghl_id_per_site.sql": "126_source_id_per_site.sql",
      "130_contacts_ghl_raw.sql": "130_contacts_source_raw.sql",
    };
    for (const [oldName, newName] of Object.entries(RENAMED_MIGRATIONS)) {
      await pool.query("UPDATE schema_migrations SET filename = $1 WHERE filename = $2", [newName, oldName]);
    }
