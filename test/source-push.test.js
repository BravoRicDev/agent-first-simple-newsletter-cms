...
// ─────────────────────────────────────────────────────────────────────────
// Sync bidirezionale con CRM sorgente + "fire unico" in cluster.
//
// Verifica:
//  1. ANTI-ECHO: una mutata con origine 'source_in'/'import' NON crea righe
//     nella push queue (evita cascade A→sorgente→A).
//  2. processPushQueue invia il contatto a sorgente (PUT /contacts/upsert),
...
describe("source-sync bidirezionale + single-fire cluster", () => {
  let site, user, server, baseUrl;
  const received = [];
  let webhookOutId = null;

  before(async () => {
    site = await createTestSite("sorgente Push Test");
    ...
    app.post("/contacts/upsert", (req, res) => {
      ...
      res.json({ contact: { id: "source-contact-" + crypto.randomBytes(4).toString("hex") } });
    });
    ...
    app.post("/opportunities", (req, res) => {
      ...
      res.json({ opportunity: { id: "source-opp-" + crypto.randomBytes(4).toString("hex") } });
    });
    ...
    const wh = (
      await query(
        `INSERT INTO webhooks (site_id, name, direction, url, secret, events, active)
         VALUES ($1, 'out-source', 'out', $2, '', '["contact_created"]', true)
         RETURNING id`,
  ...
  test("anti-echo: mutata origin source_in non crea righe in source_push_queue", async () => {
    await upsertContact(site.id, "echo@example.test", { origin: "source_in" });
    await addContactTag(site.id, "echo@example.test", "lead", { origin: "source_in" });
    await new Promise((r) => setTimeout(r, 50));
    const rows = (await query(
      "SELECT * FROM source_push_queue WHERE site_id = $1 AND entity_type = 'contact'",
      [site.id]
    )).rows;
    assert.equal(rows.length, 0, "nessuna riga push per mutate originate da sorgente");
  });
  ...
  test("processPushQueue invia il contatto a sorgente e salva external_id", async () => {
    ...
    assert.equal(afterCount, beforeCount + 1, "sorgente deve ricevere esattamente una upsert");

    const updated = (await query(
      "SELECT source_id FROM contacts WHERE id=$1", [contact.id]
    )).rows[0];
    assert.match(updated.source_id, /^source-contact-/, "source_id salvato dal push");
  ...
  test("fire-unico: processPushQueue concorrenti non duplicano l'invio", async () => {
    ...
    const afterCount = received.filter((r) => r.path === "/contacts/upsert").length;
    assert.equal(afterCount, beforeCount + 1, "esattamente UNA upsert verso sorgente nonostante 3 worker concorrenti");
    const rows = (await query(
