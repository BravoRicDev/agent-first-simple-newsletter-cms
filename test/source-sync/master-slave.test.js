...
// simmetrica quando i conteggi erano uguali ⇒ nessuno chamava più sorgente).
...
async function seedContacts(siteId, n) {
  ...
  for (let i = 0; i < n; i++) {
    await query(
      `INSERT INTO contacts (site_id, email, source_id, created_at, updated_at)
       VALUES ($1,$2,$3,NOW(),NOW())`,
      [siteId, `ms-${esuf}-${i}@example.test`, `source-${esuf}-${i}`]
    );
  }
}
