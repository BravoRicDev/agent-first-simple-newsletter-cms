...
async function seedSet(siteId) {
  const sfx = rnd();
  const mk = (name) => ({ id: `source-${sfx}-${name}`, email: `${name}-${sfx}@example.test` });
  ...
  await query(
    "INSERT INTO contacts (site_id, email, source_id, created_at, updated_at) VALUES ($1,$2,$3,$4,$5)",
...
describe("source-sync: caccia sotto-risorse selettiva + sweep periodico", () => {
  ...
  test("non-due e tutto unchanged: onPage MAI chiamato (costo ~0 in regime stabile)", async () => {
    ...
    const sfx = rnd();
    const c = { id: `source-${sfx}-only`, email: `only-${sfx}@example.test` };
    await query(
      "INSERT INTO contacts (site_id, email, source_id, created_at, updated_at) VALUES ($1,$2,$3,$4,$5)",
