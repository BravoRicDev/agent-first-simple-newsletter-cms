...
// avere SIA external_id proprio (uuid DEFAULT gen_random_uuid()) SIA source_id
...
// NOTA: sul nostro account sorgente il sorgente GET /payments/ è bloccato da IAM
...
describe("Clone API — Payment links (round 20, doppio id)", () => {
  ...
  let p1Uuid, p2Uuid;
  const p1SourceId = "paySource" + crypto.randomBytes(7).toString("hex"); // stile id reale sorgente
  ...
  before(async () => {
    ...
    // p1: sincronizzato da sorgente (source_id reale) — external_id dal default
    const r1 = await query(
      `INSERT INTO payment_links (site_id, source_id, title, amount, currency, contact_email, status, stripe_url)
    ...
    // p2: creato localmente (solo UUID, source_id vuoto)
    ...
    await query(
      "INSERT INTO payment_links (site_id, source_id, title, amount) VALUES ($1, $2, 'Altrui', 5)",
  ...
  test("GET /payments → lista: riga sincronizzata espone source_id reale, riga locale espone UUID", async () => {
    ...
    const s1 = res.data.payments.find((p) => p.id === p1SourceId);
    assert.ok(s1, "riga sincronizzata: id = source_id REALE (publicId preferisce source_id)");
    ...
    assert.ok(s2, "riga locale senza source_id: id = UUID interno (fallback)");
    assert.equal(s2.description, "senza sync");
  });

  test("GET /payments/:id → risolve SIA source_id reale SIA UUID interno (stesso record)", async () => {
    const bySource = await fetch(`/payments/${p1SourceId}`);
    assert.equal(bySource.status, 200);
    assert.equal(bySource.data.payment.id, p1SourceId);

    const byUuid = await fetch(`/payments/${p1Uuid}`);
    assert.equal(byUuid.status, 200);
    assert.equal(byUuid.data.payment.id, p1SourceId, "UUID risolve, ma id esposto resta il source_id");
    assert.equal(byUuid.data.payment.title, "Link corso base");
  });

  test("GET /payments con cursore: nextPage = publicId, accetta source_id e UUID come startAfterId", async () => {
    const p = await fetch("/payments?limit=1");
    assert.equal(p.status, 200);
    assert.equal(p.data.payments.length, 1);
    assert.equal(p.data.meta.nextPage, p1SourceId, "nextPage = id esposto dell'ultima riga");

    const nextBySource = await fetch(`/payments?limit=5&startAfterId=${p1SourceId}`);
    assert.equal(nextBySource.status, 200);
    assert.deepEqual(nextBySource.data.payments.map((x) => x.id), [p2Uuid]);
  ...
  test("Errori: inesistente → 404, 300 char → 400, payment di altro sito non visibile", async () => {
    const missing = await fetch("/payments/sourcePAYMENTinesistente");
  ...
  test("Sola lettura: POST/PUT/DELETE /payments → 404 (link reale emesso solo dal processor sorgente)", async () => {
    ...
    const put = await fetch(`/payments/${p1SourceId}`, { method: "PUT", body: JSON.stringify({ title: "x" }) });
    assert.equal(put.status, 404);
    const del = await fetch(`/payments/${p1SourceId}`, { method: "DELETE" });
