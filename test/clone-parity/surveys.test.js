...
describe("Onda D — Surveys clone", () => {
  ...
  // Parity source_id: "not-a-uuid" è un formato di id valido (potrebbe essere
  // un source_id reale) — requireAnyId/findByAnyId lo accettano e rispondono
  ...
  test("Parity source_id: round-trip GET/PUT/DELETE survey col source_id reale", async () => {
    const createRes = await fetch("/surveys", {
      method: "POST",
    ...
    const realSourceId = "sourceSURVEYparity001";
    await query("UPDATE surveys SET source_id = $1 WHERE external_id = $2", [realSourceId, created.id]);

    const getRes = await fetch(`/surveys/${realSourceId}`);
    assert.equal(getRes.status, 200);
    assert.equal(getRes.data.survey.id, realSourceId, "id risposta deve essere il source_id reale");

    const putRes = await fetch(`/surveys/${realSourceId}`, {
      method: "PUT",
    ...
    const deleteRes = await fetch(`/surveys/${realSourceId}`, { method: "DELETE" });
    assert.equal(deleteRes.status, 200);

    const getAfterDel = await fetch(`/surveys/${realSourceId}`);
    assert.equal(getAfterDel.status, 404);
  });

  test("Parity source_id: submission espone surveyId/contactId reali quando presenti", async () => {
    const createRes = await fetch("/surveys", {
      method: "POST",
      body: JSON.stringify({ name: "RtSurveyForSub" }),
    });
    const created = createRes.data.survey;
    const realSurveySourceId = "sourceSURVEYforsub001";
    await query("UPDATE surveys SET source_id = $1 WHERE external_id = $2", [realSurveySourceId, created.id]);
    ...
    const submitRes = await fetch(`/surveys/${realSurveySourceId}/submissions`, {
      method: "POST",
      body: JSON.stringify({
        contactId: realContactSourceId,
        answers: { feedback: "source_id round-trip" },
      }),
    });
    assert.equal(submitRes.status, 201);
    assert.equal(submitRes.data.submission.surveyId, realSurveySourceId);
    assert.equal(submitRes.data.submission.contactId, realContactSourceId);
  });
});
