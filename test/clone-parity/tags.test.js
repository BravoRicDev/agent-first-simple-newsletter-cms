...
describe("Clone API — Tags (Onda A)", () => {
  ...
  // Parity source_id: "invalid-uuid" è un formato di id valido (potrebbe essere
  // un source_id reale) — requireAnyId/findByAnyId lo accettano e rispondono 404
  ...
  // Round-trip test: tag creato via API, source_id sovrascritto in DB,
  test("round-trip GET/PUT/DELETE col source_id reale", async () => {
    ...
    // 2) Sovrascrivi il suo source_id nel DB con un valore alfanumerico reale sorgente
    // NOTA: created.id è il campo "id" esposto in risposta (external_id, dato
    // che source_id è ancora vuoto alla creazione) — NON la PK intera "id" della
    // tabella tags, quindi il match va fatto su external_id.
    const realSourceId = "source_12345_abcde";
    await query(
      `UPDATE tags SET source_id = $1 WHERE external_id = $2`,
      [realSourceId, created.id]
    );

    // 3) GET col source_id reale al posto dell'UUID deve restituire 200
    const getRes = await fetch(url(`/tags/${realSourceId}`), {
      headers: auth(),
    });
    assert.equal(getRes.status, 200);
    const body = await getRes.json();
    assert.equal(body.tag.id, realSourceId, "id risposta deve essere il source_id reale");
    assert.equal(body.tag.name, "RtTag");

    // 4) PUT col source_id reale deve funzionare
    const putRes = await fetch(url(`/tags/${realSourceId}`), {
      method: "PUT",
      headers: auth(),
    ...
    // 5) DELETE col source_id reale deve funzionare
    const deleteRes = await fetch(url(`/tags/${realSourceId}`), {
      method: "DELETE",
      headers: auth(),
    });
    assert.equal(deleteRes.status, 200);
    ...
    const getAfterDel = await fetch(url(`/tags/${realSourceId}`), {
      headers: auth(),
    });
