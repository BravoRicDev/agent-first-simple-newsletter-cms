...
describe("Onda A — Opportunità clone", () => {
  ...
  test("PUT /opportunities/:id — aggiorna opportunità + lastStatusChangeAt", async () => {
    const createRes = await fetch(`${baseUrl}/opportunities`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    assert.ok(body.opportunity.lastStatusChangeAt); // sorgente shape: lastStatusChangeAt (not lastStatusChange)
    assert.ok(body.traceId); // sorgente shape includes traceId
  });
  ...
  test("GET /opportunities/search — cerca opportunità con query params (snake_case, shape meta-wrapped)", async () => {
    const searchRes = await fetch(`${baseUrl}/opportunities/search?status=open&limit=5`);
    assert.equal(searchRes.status, 200);
    const body = await searchRes.json();
    assert.ok(Array.isArray(body.opportunities));
    assert.ok(body.meta);
    assert.equal(typeof body.meta.total, "number");
    assert.ok(body.traceId); // sorgente shape: traceId
  });

  test("GET /opportunities/search — cerca per contact_id (snake_case)", async () => {
    const searchRes = await fetch(`${baseUrl}/opportunities/search?contact_id=${contact.externalId}`);
    assert.equal(searchRes.status, 200);
    const body = await searchRes.json();
    assert.ok(Array.isArray(body.opportunities));
    assert.ok(body.meta);
  ...
  test("GET /opportunities/:id — sorgente shape con traceId", async () => {
    const createRes = await fetch(`${baseUrl}/opportunities`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: "sorgente Shape Test",
        contactId: contact.externalId,
        status: "open",
        monetaryValue: 3000,
      }),
    ...
    assert.ok(body.opportunity);
    assert.ok(body.opportunity.id);
    assert.equal(body.opportunity.name, "sorgente Shape Test");
    assert.equal(body.opportunity.status, "open");
    assert.equal(body.opportunity.monetaryValue, 3000);
    // sorgente shape fields
    assert.ok(body.opportunity.createdAt);
    assert.ok(body.opportunity.updatedAt);
    assert.ok(body.opportunity.locationId);
    assert.ok(Array.isArray(body.opportunity.customFields));
    assert.ok(Array.isArray(body.opportunity.followers));
    assert.ok(Array.isArray(body.opportunity.sort));
    assert.ok(body.traceId); // sorgente shape: traceId
  });
