...
describe("Onda H — Commerce clone", () => {
  ...
  // ── Parity source_id (round 13) ──────────────────────────────────────────

  test("Parity source_id: round-trip GET/PUT/DELETE product col source_id reale", async () => {
    const createRes = await fetch("/products", {
      method: "POST",
      body: JSON.stringify({
        name: "RtProduct",
    ...
    const realSourceId = "sourcePRODparity000001";
    await query("UPDATE products SET source_id = $1 WHERE external_id = $2", [realSourceId, uuidId]);

    // GET col source_id reale → 200 e id = source_id reale
    const getRes = await fetch(`/products/${realSourceId}`);
    assert.equal(getRes.status, 200);
    assert.equal(getRes.data.product.id, realSourceId, "id risposta deve essere il source_id reale");

    // PUT col source_id reale
    const putRes = await fetch(`/products/${realSourceId}`, {
      method: "PUT",
      body: JSON.stringify({ name: "RtProduct Updated" }),
    });
    assert.equal(putRes.status, 200);
    assert.equal(putRes.data.product.name, "RtProduct Updated");
    assert.equal(putRes.data.product.id, realSourceId);
    // Anello dei prezzi: anche il price id è esposto col proprio source_id/UUID
    ...
    assert.equal(getByUuid.data.product.id, realSourceId, "UUID interno risolve, ma id resta il source_id");

    // DELETE col source_id reale
    const delRes = await fetch(`/products/${realSourceId}`, { method: "DELETE" });
    assert.equal(delRes.status, 200);
    const goneRes = await fetch(`/products/${realSourceId}`);
    assert.equal(goneRes.status, 404);
  });

  test("Parity source_id: round-trip GET/PUT/DELETE invoice col source_id reale + contactId source_id", async () => {
    // Contatto con source_id reale: deve essere accettabile in input E esposto in output
    const contactSourceId = "sourceCONTACTinv00001";
    await query("UPDATE contacts SET source_id = $1 WHERE id = $2", [contactSourceId, contact.id]);

    const createRes = await fetch("/invoices", {
      method: "POST",
      body: JSON.stringify({
        contactId: contactSourceId,
        items: [{ description: "Riga", quantity: 1, unitPrice: 70 }],
      }),
    });
    assert.equal(createRes.status, 201);
    assert.equal(createRes.data.invoice.contactId, contactSourceId, "contactId in output = source_id reale");
    const uuidId = createRes.data.invoice.id;

    const realSourceId = "sourceINVOICEparity001";
    await query("UPDATE invoices SET source_id = $1 WHERE external_id = $2", [realSourceId, uuidId]);

    const getRes = await fetch(`/invoices/${realSourceId}`);
    assert.equal(getRes.status, 200);
    assert.equal(getRes.data.invoice.id, realSourceId, "id risposta deve essere il source_id reale");
    assert.equal(getRes.data.invoice.contactId, contactSourceId);
    // Item: id esposto sempre via publicId (source_id se presente, altrimenti UUID)
    assert.ok(getRes.data.invoice.items[0].id);

    const putRes = await fetch(`/invoices/${realSourceId}`, {
      method: "PUT",
    ...
    // Filtro per contactId col source_id reale del contatto
    const listRes = await fetch(`/invoices?contactId=${contactSourceId}`);
    assert.equal(listRes.status, 200);
    const found = listRes.data.invoices.filter((i) => i.id === realSourceId);
    assert.equal(found.length, 1, "la fattura deve comparire filtrando per contactId=source_id");

    // DELETE col source_id reale (draft → permesso)
    const delRes = await fetch(`/invoices/${realSourceId}`, { method: "DELETE" });
    assert.equal(delRes.status, 200);
  });

  test("Parity source_id: id malformato (300 char) → 400 su products e invoices", async () => {
    ...
    const p404 = await fetch("/products/sourceINESISTENTE000000001");
    assert.equal(p404.status, 404);
    const i404 = await fetch("/invoices/sourceINESISTENTE00000001");
