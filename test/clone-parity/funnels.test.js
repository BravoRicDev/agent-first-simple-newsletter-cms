...
// Round 17: Funnels sorgente — clone API sola lettura su source_funnels (copia del
// sorgente GET /funnels/funnel/list sincronizzata dal mapper "funnels").
// Verificato sullo schema reale: source_funnels NON ha external_id proprio →
// l'unico id pubblico è source_id (quello reale di sorgente, 20 char alfanumerici):
// niente doppio id, pattern "mirror" come source_workflows (round 16).
describe("Clone API — Funnels sorgente (round 17, sola lettura)", () => {
  let server;
  let site;
  let otherSite;
  let apiKey;
  ...
  // id reali stile sorgente (20 char alfanumerici, NON uuid-valid)
  const fn1 = {
    sourceId: "1g9OWTij9iU9yzKXOyWb",
  ...
  before(async () => {
    ...
    await query(
      `INSERT INTO source_funnels (site_id, source_id, name, steps)
       VALUES ($1, $2, $3, $4), ($1, $5, $6, $7), ($8, $9, $10, $11)`,
      [
        site.id, fn1.sourceId, fn1.name, JSON.stringify(fn1.steps),
        fn2.sourceId, fn2.name, JSON.stringify(fn2.steps),
  ...
  test("GET /funnels → lista con id = source_id reale e steps integrali, solo il proprio sito", async () => {
    ...
    const served1 = res.data.funnels.find((f) => f.id === fn1.sourceId);
    assert.ok(served1, "fn1 presente con il suo source_id REALE come id");
  ...
  test("GET /funnels/:id → singolo funnel per source_id reale", async () => {
    const res = await fetch(`/funnels/${fn2.sourceId}`);
    assert.equal(res.status, 200);
    assert.equal(res.data.funnel.id, fn2.sourceId);
    assert.deepEqual(res.data.funnel.steps, fn2.steps);
  });

  test("GET /funnels con cursore: nextPage = source_id, pagina 2 coerente", async () => {
    const p1 = await fetch("/funnels?limit=1");
    assert.equal(p1.status, 200);
    assert.equal(p1.data.funnels.length, 1);
    assert.equal(p1.data.funnels[0].id, fn1.sourceId);
    assert.equal(p1.data.meta.nextPage, fn1.sourceId, "nextPage = source_id dell'ultimo restituito");

    const p2 = await fetch(`/funnels?limit=1&startAfterId=${fn1.sourceId}`);
    assert.equal(p2.status, 200);
    assert.equal(p2.data.funnels.length, 1);
    assert.equal(p2.data.funnels[0].id, fn2.sourceId, "il cursore source_id scorre davvero");
  });

  test("Errori: id inesistente → 404, id malformato (300 char) → 400, funnel di altro sito non visibile", async () => {
    const missing = await fetch("/funnels/sourceFUNNELinesistente001");
  ...
  test("Sola lettura: POST/PUT/DELETE /funnels → 404 (funnel = pubblicazione editor sorgente, non replicabile qui)", async () => {
    const post = await fetch("/funnels", { method: "POST", body: JSON.stringify({ name: "x" }) });
    assert.equal(post.status, 404);
    const put = await fetch(`/funnels/${fn1.sourceId}`, { method: "PUT", body: JSON.stringify({ name: "x" }) });
    assert.equal(put.status, 404);
    const del = await fetch(`/funnels/${fn1.sourceId}`, { method: "DELETE" });
