...
// Round 16: Workflows sorgente — clone API in sola lettura su source_workflows
// (copia dei payload REALI sincronizzati dal mapper source-sync
// "source-workflows"). L'unico id è source_id (quello reale di sorgente): la tabella
...
describe("Clone API — Workflows sorgente (round 16, sola lettura)", () => {
  let server;
  let site;
  let otherSite;
  let apiKey;
  ...
  // Payload REAListici stile risposta sorgente GET /workflows/
  const wf1 = {
  ...
  before(async () => {
    ...
    await query(
      `INSERT INTO source_workflows (site_id, source_id, name, status, payload)
  ...
  test("GET /workflows → lista con payload integrali e meta, solo il proprio sito", async () => {
    ...
    // Parità byte-per-byte: i payload serviti sono identici a quelli sorgente
    ...
    // Campi originali sorgente conservati (steps/triggers annidati inclusi)
    assert.equal(served1.steps[0].type, "sms");
    assert.equal(served1.triggers[0].type, "contact_created");
  });

  test("GET /workflows/:id → workflow singolo per source_id reale, payload integrale", async () => {
    const res = await fetch(`/workflows/${wf1.id}`);
    assert.equal(res.status, 200);
    assert.deepEqual(res.data.workflow, wf1);
  });

  test("GET /workflows con cursore: limit=1 → nextPage = source_id, pagina 2 coerente", async () => {
    ...
    assert.equal(p1.data.meta.nextPage, wf1.id, "nextPage = source_id reale dell'ultimo restituito");

    const p2 = await fetch(`/workflows?limit=1&startAfterId=${wf1.id}`);
    assert.equal(p2.status, 200);
    assert.equal(p2.data.workflows.length, 1);
    assert.equal(p2.data.workflows[0].id, wf2.id, "il cursore source_id scorve davvero");
  });

  test("Errori: id inesistente → 404, id malformato (300 char) → 400, workflow di altro sito non visibile", async () => {
    const missing = await fetch("/workflows/sourceWORKFLOWinesistente1");
  ...
  test("Sola lettura: POST/PUT/DELETE /workflows → 404 (niente scrittura: engine sorgente reale non replicabile qui)", async () => {
