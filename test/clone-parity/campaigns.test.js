...
describe("Onda E — Campagne clone", () => {
  ...
  test("Parity source_id: round-trip GET/PUT/DELETE campagna col source_id reale", async () => {
    const createRes = await fetch("/campaigns", {
      method: "POST",
    ...
    const realSourceId = "sourceCAMPAIGNparity001";
    await query("UPDATE newsletter_campaigns SET source_id = $1 WHERE external_id = $2", [realSourceId, created.id]);

    const getRes = await fetch(`/campaigns/${realSourceId}`);
    assert.equal(getRes.status, 200);
    assert.equal(getRes.data.campaign.id, realSourceId, "id risposta deve essere il source_id reale");

    const putRes = await fetch(`/campaigns/${realSourceId}`, {
      method: "PUT",
    ...
    const deleteRes = await fetch(`/campaigns/${realSourceId}`, { method: "DELETE" });
    assert.equal(deleteRes.status, 200);

    const getAfterDel = await fetch(`/campaigns/${realSourceId}`);
    assert.equal(getAfterDel.status, 404);
  });

  test("Parity source_id: id campagna malformato (300 char) → 400", async () => {
    const res = await fetch(`/campaigns/${"x".repeat(300)}`);
    assert.equal(res.status, 400);
  });

  test("Parity source_id: round-trip GET/PUT/DELETE template col source_id reale", async () => {
    const createRes = await fetch("/templates", {
      method: "POST",
    ...
    const realSourceId = "sourceTEMPLATEparity001";
    await query("UPDATE marketing_templates SET source_id = $1 WHERE external_id = $2", [realSourceId, created.id]);

    const getRes = await fetch(`/templates/${realSourceId}`);
    assert.equal(getRes.status, 200);
    assert.equal(getRes.data.template.id, realSourceId, "id risposta deve essere il source_id reale");

    const deleteRes = await fetch(`/templates/${realSourceId}`, { method: "DELETE" });
    assert.equal(deleteRes.status, 200);
  });

  test("Parity source_id: subscription espone campaignId/contactId reali quando presenti", async () => {
    const campRes = await fetch("/campaigns", {
      method: "POST",
      body: JSON.stringify({ name: "SubSource", subject: "Sub", content: "Test" }),
    });
    const created = campRes.data.campaign;
    const realCampaignSourceId = "sourceCAMPAIGNforsub001";
    await query("UPDATE newsletter_campaigns SET source_id = $1 WHERE external_id = $2", [realCampaignSourceId, created.id]);

    const realContactSourceId = "sourceCONTACTforsub001";
    await query("UPDATE contacts SET source_id = $1 WHERE id = $2", [realContactSourceId, contact.id]);

    const addRes = await fetch(`/contacts/${realContactSourceId}/campaigns/${realCampaignSourceId}`, {
      method: "POST",
    });
    assert.equal(addRes.status, 201);
    assert.equal(addRes.data.subscription.campaignId, realCampaignSourceId);
    assert.equal(addRes.data.subscription.contactId, realContactSourceId);
  });
});
