...
describe("Onda H — Memberships clone", () => {
  ...
  // ── Parity source_id (round 14) ───────────────────────────────────────────
  // contacts HA source_id (sincronizzato da sorgente): l'automazione può usare
  // l'id reale del contatto sia in input (contactId) sia in output.
  // memberships/courses/enrollments NON hanno source_id → i loro id restano
  // UUID-only (verificato: colonna assente in information_schema).

  test("Parity source_id: enrollment accetta contactId=source_id e lo espone in output", async () => {
    // Dai un source_id reale al contatto condiviso
    const contactSourceId = "sourceCONTACTenroll001";
    ...
    const mRes = await fetch("/memberships", {
      method: "POST",
      body: JSON.stringify({ name: "SourceId Course", description: "d" }),
    });
    assert.equal(mRes.status, 201);
    const membershipId = mRes.data.membership.id;

    // Enroll passando il source_id reale del contatto (accettato da findByAnyId)
    const enrollRes = await fetch(`/memberships/${membershipId}/enroll`, {
      method: "POST",
      body: JSON.stringify({ contactId: contactSourceId }),
    });
    assert.equal(enrollRes.status, 201);
    assert.equal(
      enrollRes.data.enrollment.contactId,
      contactSourceId,
      "contactId in output deve essere il source_id reale"
    );

    // Anche la lista lo espone col source_id
    const listRes = await fetch(`/memberships/${membershipId}/enrollments`);
    assert.equal(listRes.status, 200);
    const mine = listRes.data.enrollments.filter((e) => e.membershipId === membershipId);
    assert.equal(mine.length >= 1, true);
    assert.equal(mine[0].contactId, contactSourceId, "contactId in lista = source_id reale");
  });
});
