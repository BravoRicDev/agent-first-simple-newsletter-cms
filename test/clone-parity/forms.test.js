...
describe("Onda C — Forms clone", () => {
  ...
  // Parity source_id: round-trip su form (submission id reale esposto quando
  // presente, id malformato/inesistente gestiti correttamente).
  test("Parity source_id: round-trip GET/PUT/DELETE form col source_id reale", async () => {
    const createRes = await fetch(`${baseUrl}/forms`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const realSourceId = "sourceFORMparity001";
    await query(`UPDATE forms SET source_id = $1 WHERE external_id = $2`, [realSourceId, created.id]);

    const getRes = await fetch(`${baseUrl}/forms/${realSourceId}`);
    assert.equal(getRes.status, 200);
    const got = (await getRes.json()).form;
    assert.equal(got.id, realSourceId, "id risposta deve essere il source_id reale");

    const putRes = await fetch(`${baseUrl}/forms/${realSourceId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
    ...
    const deleteRes = await fetch(`${baseUrl}/forms/${realSourceId}`, { method: "DELETE" });
    assert.equal(deleteRes.status, 200);

    const getAfterDel = await fetch(`${baseUrl}/forms/${realSourceId}`);
    assert.equal(getAfterDel.status, 404);
  });

  test("Parity source_id: id form malformato (300 char) → 400", async () => {
    const res = await fetch(`${baseUrl}/forms/${"x".repeat(300)}`);
    assert.equal(res.status, 400);
  });

  test("Parity source_id: id form formato valido ma inesistente → 404", async () => {
    const res = await fetch(`${baseUrl}/forms/nonexistent-source-id`);
    assert.equal(res.status, 404);
  });

  test("Parity source_id: submission espone formId/contactId reali quando presenti", async () => {
    const realFormSourceId = "sourceFORMforsub001";
    const realContactSourceId = "sourceCONTACTforsub001";
    await query(`UPDATE forms SET source_id = $1 WHERE id = $2`, [realFormSourceId, form.id]);
    ...
    assert.equal(sub.formId, realFormSourceId);
    assert.equal(sub.contactId, realContactSourceId);
  });
});
