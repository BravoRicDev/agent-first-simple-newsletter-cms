...
describe("Clone API — Custom Fields (Onda A)", () => {
  ...
  // Parity source_id: round-trip su custom field (le folder NON hanno source_id,
  // sono una feature puramente locale mai sincronizzata da sorgente — nessun fix
  // necessario lì).
  test("Parity source_id: round-trip GET/PUT/DELETE col source_id reale + id esposto", async () => {
    const createRes = await fetch(url("/custom-fields"), {
      method: "POST",
      headers: auth(),
    ...
    const realSourceId = "sourceFIELDparity001";
    await query(`UPDATE custom_fields SET source_id = $1 WHERE external_id = $2`, [realSourceId, created.id]);

    const getRes = await fetch(url(`/custom-fields/${realSourceId}`), { headers: auth() });
    assert.equal(getRes.status, 200);
    const got = (await getRes.json()).customField;
    assert.equal(got.id, realSourceId, "id risposta deve essere il source_id reale");

    const putRes = await fetch(url(`/custom-fields/${realSourceId}`), {
      method: "PUT",
      headers: auth(),
    ...
    const deleteRes = await fetch(url(`/custom-fields/${realSourceId}`), {
      method: "DELETE",
      headers: auth(),
    });
    assert.equal(deleteRes.status, 200);

    const getAfterDel = await fetch(url(`/custom-fields/${realSourceId}`), { headers: auth() });
    assert.equal(getAfterDel.status, 404);
  });

  test("Parity source_id: id malformato (300 char) → 400", async () => {
    const res = await fetch(url(`/custom-fields/${"x".repeat(300)}`), { headers: auth() });
    assert.equal(res.status, 400);
  });

  test("Parity source_id: id formato valido ma inesistente → 404", async () => {
    const res = await fetch(url("/custom-fields/nonexistent-source-id"), { headers: auth() });
    assert.equal(res.status, 404);
  });

  // ── Round 18: CUSTOM VALUES dentro GET /custom-fields ─────────────────
  // sorgente serve i custom values nella STESSA risposta di GET /customFields/
  // (chiave `customValues`). source_custom_values è mirror del sorgente:
  // nessun external_id proprio → id = source_id reale, niente doppio id.
  // Nessun endpoint separato /customValues/:id in sorgente → niente da fixare
  ...
  test("Round 18: GET /custom-fields include customValues con source_id reale e scope per sito", async () => {
    const other = await createTestSite("CF Values Other");

    // source_id casuali per run: il volume di test persiste tra esecuzioni e
    // source_custom_values ha UNIQUE(site_id, source_id)
    ...
    // 2 valori sul nostro sito (source_id reali stile sorgente) + 1 su altro sito
    await query(
      `INSERT INTO source_custom_values (site_id, source_id, name, value)
    ...
    // customValues presente, SOLO valori del proprio sito, id = source_id reale
    assert(Array.isArray(body.customValues), "chiave customValues sempre presente (come sorgente)");
    assert.equal(body.customValues.length, 2, "solo i valori del sito corrente");
    const v1 = body.customValues.find((v) => v.id === cv1Id);
    assert.ok(v1, "valore presente con source_id REALE come id");
