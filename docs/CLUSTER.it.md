...
Oltre all'**import** sorgente→CMS (`source_sync_config`), è disponibile il **push**
CMS→sorgente (contatti e opportunità), **opzionale** e disattivato di default.
...
- **Single-fire in cluster**: `services/source-sync/push.js` garantisce che
  in ogni finestra UN SOLO nodo sparì a sorgente: lease per-sito (`advisory lock`),
...
- **Anti-cascata**: le mutate con origine `source_in`/`import` (arrivate da sorgente)
  **non vengono mai rispedite** a sorgente → niente loop A→sorgente→A→sorgente….
- **Monitor**: `GET /api/agent/sites/:siteId/source-push` (stato coda),
  `POST .../source-push/run` (run manuale).
