...
> **Sync bidirezionale con il CRM sorgente (opzionale)**: oltre all'import sorgente→CMS (source-sync), il CMS può *spingere* le modifiche di contatti/opportunità di ritorno al CRM. Attivazione per-sito (`source_sync_config.push_enabled` + `pushDirection`, via agent API). Outbox cluster-safe (`source_push_queue`) con garanzia "single-fire": un solo nodo spara per finestra, mai durante un import in corso o con replica indietro, e le mutate originate da sorgente non vengono mai rispedite (niente cascate di automazioni). Vedi [`docs/CLUSTER.it.md`](docs/CLUSTER.it.md) §8bis.
