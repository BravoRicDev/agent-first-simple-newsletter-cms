...
//
// Parity source_id: ogni id esposto (opportunità, pipeline, pipeline stage,
// contatto, utente) preferisce il source_id reale del CRM sorgente quando
...
export async function serializeOpportunity(row, pipelineStageId = null, contactId = null, assignedToId = null, locationId = null) {
  ...
  // Costruisci l'oggetto contact nested (sorgente shape per GET /opportunities/:id)
  // solo se i dati del contatto sono disponibili nel row (arricchito da getOpportunity).
  let contactObj = null;
  if (row._contact_name || row._contact_email) {
    contactObj = {
      id: contactId || null,
      name: row._contact_name || "",
      companyName: row._contact_company || "",
      email: row._contact_email || "",
      phone: row._contact_phone || "",
      tags: row._contact_tags || [],
      score: null,
      assignedTo: row._contact_assigned_to || null,
      followers: row._contact_followers || [],
    };
  }

  // Custom fields dal JSONB opportunities.custom_fields (array [{source_id,field_value,type}]).
  // sorgente GET /opportunities/:id risponde con [{id,fieldValue}], search con [{id,fieldValueDate,type}].
  // Qui produciamo il shape GET (id,fieldValue) per coprire il caso più comune.
  let customFields = [];
  try {
    const rawCf = typeof row.custom_fields === "string" ? JSON.parse(row.custom_fields) : row.custom_fields;
    if (Array.isArray(rawCf)) {
      customFields = rawCf.map((cf) => ({
        id: cf.source_id || cf.external_id || cf.id,
        fieldValue: cf.field_value !== undefined ? cf.field_value : null,
        type: cf.type || null,
      }));
    }
  } catch { /* ignora parsing error */ }

  return {
    id: publicId(row),
    locationId,
    name: row.title || "",
    pipelineId: publicId({ source_id: row.pipeline_source_id, external_id: row.pipeline_external_id }),
    pipelineStageId,
    ...
    isAttribute: null, // non disponibile nel clone locale
    internalSource: null, // non disponibile nel clone locale
    lastStatusChangeAt: row.last_status_change?.toISOString?.() || row.last_status_change || null,
    lastStageChangeAt: row.last_stage_change_at?.toISOString?.() || row.last_stage_change_at || null,
    createdAt: row.created_at?.toISOString() || null,
    updatedAt: row.updated_at?.toISOString() || null,
    forecastProbability: row.probability || null,
    effectiveProbability: null, // non disponibile nel clone locale
    customFields,
    followers: [],
    sort: [],
    attributions: [],
    contact: contactObj,
    lostReasonId: row.lost_reason_id || null,
    lastActionDate: row.last_action_date?.toISOString?.() || row.last_action_date || null,
  };
}
...
// Risolvi uno stage (key) → pipelineStageId (source_id reale o uuid) da
...
async function resolveOrCreatePipelineStage(siteId, pipelineId, stageKey) {
  ...
  const existing = (await query(
    `SELECT external_id, source_id FROM pipeline_stages WHERE pipeline_id = $1 AND key = $2`,
    [pipelineId, stageKey]
  )).rows[0];
  ...
  const inserted = (await query(
    `INSERT INTO pipeline_stages (pipeline_id, key, label, position)
     ...
     RETURNING external_id, source_id`,
    [pipelineId, stageKey, stageKey]
  )).rows[0];
  return inserted ? publicId(inserted) : null;
}

// Risolvi contactEmail → contactId (source_id reale o external_id).
// Ritorna null se contatto non trovato.
async function resolveContactId(siteId, contactEmail) {
  if (!contactEmail) return null;
  const row = (await query(
    `SELECT external_id, source_id FROM contacts WHERE site_id = $1 AND LOWER(email) = LOWER($2)`,
    [siteId, contactEmail]
  )).rows[0];
  return row ? publicId(row) : null;
}

// Risolvi assignedTo (user_id interno) → assignedTo (source_id reale o uuid).
async function resolveAssignedToId(siteId, userId) {
  if (!userId) return null;
  const row = (await query(
    `SELECT external_id, source_id FROM users WHERE id = $1 AND site_id = $2`,
    [userId, siteId]
  )).rows[0];
  return row ? publicId(row) : null;
}

// Risolvi contactId (UUID o source_id reale) → contact id interno.
...
// Risolvi pipelineId (UUID o source_id reale) → pipeline id interno.
...
// Risolvi pipelineStageId (UUID o source_id reale) → (pipeline_id, stage key).
...
// account sorgente possono condividere lo stesso source_id di stage, vedi
// db/126_source_id_per_site.sql: indice composito su (pipeline_id, source_id),
// non (site_id, source_id)).
async function resolvePipelineStageInternal(siteId, pipelineStageIdAny) {
  ...
  const row = (await query(
    `SELECT ps.pipeline_id, ps.key FROM pipeline_stages ps
     ...
     WHERE ps.external_id::text = $2 OR ps.source_id = $2`,
...
// Risolvi assignedToId (UUID o source_id reale) → user id interno.
...
export async function listOpportunities(siteId, filters = {}, locationId = null) {
  ...
  const rows = (await query(
    `SELECT o.*, p.external_id AS pipeline_external_id, p.source_id AS pipeline_source_id
...
export async function getOpportunity(siteId, externalId, locationId = null) {
  const row = (await query(
    `SELECT o.*, p.external_id AS pipeline_external_id, p.source_id AS pipeline_source_id
     FROM opportunities o
     LEFT JOIN pipelines p ON p.id = o.pipeline_id
     WHERE o.site_id = $1 AND (o.external_id::text = $2 OR o.source_id = $2)`,
    [siteId, externalId]
  )).rows[0];
  if (!row) return null;
  ...
  // Arricchisci il row con i dati del contatto per il nested contact object (sorgente shape)
  if (row.contact_email) {
    const contactRow = (await query(
      "SELECT id, source_id, external_id, tags FROM contacts WHERE site_id = $1 AND LOWER(email) = LOWER($2) LIMIT 1",
      [siteId, row.contact_email]
    )).rows[0];
    if (contactRow) {
      // I dettagli del profilo (name, companyName, phone) sono in contact_custom_values
      const cv = (await query(
        "SELECT values FROM contact_custom_values WHERE site_id = $1 AND contact_id = $2 AND object_key = 'contact'",
        [siteId, contactRow.id]
      )).rows[0];
      const vals = cv?.values || {};
      const firstName = vals?.firstName || "";
      const lastName = vals?.lastName || "";
      row._contact_name = `${firstName} ${lastName}`.trim() || "";
      row._contact_company = vals?.companyName || "";
      row._contact_email = row.contact_email;
      row._contact_phone = vals?.phone || "";
      row._contact_tags = contactRow.tags || [];
    }
  }
...
export async function updateOpportunity(siteId, externalId, input = {}, locationId = null) {
  ...
  const internalRow = (await query(
    `SELECT id, contact_email, pipeline_id, stage, owner_id, status FROM opportunities WHERE site_id = $1 AND (external_id::text = $2 OR source_id = $2)`,
    [siteId, externalId]
  )).rows[0];
...
export async function deleteOpportunity(siteId, externalId) {
  const row = (await query(
    `DELETE FROM opportunities WHERE site_id = $1 AND (external_id::text = $2 OR source_id = $2) RETURNING id`,
    [siteId, externalId]
  )).rows[0];
  return row ? 1 : 0;
}
...
export async function setOpportunityStatus(siteId, externalId, status, locationId = null) {
  ...
  const internalRow = (await query(
    `SELECT id, status FROM opportunities WHERE site_id = $1 AND (external_id::text = $2 OR source_id = $2)`,
    [siteId, externalId]
  )).rows[0];
...
export async function upsertOpportunity(siteId, input = {}, locationId = null) {
  ...
  const existing = (await query(
    `SELECT id, external_id, source_id FROM opportunities WHERE site_id = $1 AND LOWER(contact_email) = LOWER($2) AND LOWER(title) = LOWER($3)`,
...
export async function listOpportunityFollowers(siteId, opportunityExternalId) {
  ...
  const rows = (await query(
    `SELECT f.external_id, u.external_id AS user_external_id, u.source_id AS user_source_id, u.name, u.email FROM opportunity_followers f
  ...
  return rows.map((r) => ({
    id: publicId({ external_id: r.user_external_id, source_id: r.user_source_id }),
    firstName: r.name || "",
    email: r.email || "",
  }));
}
  ...
  return Promise.all(
    rows.map(async (p) => {
      const stageRows = (await query(
        `SELECT external_id, source_id, label FROM pipeline_stages WHERE pipeline_id = $1 ORDER BY position ASC`,
        [p.id]
      )).rows;
  ...
  if (Array.isArray(input.stages)) {
    for (let i = 0; i < input.stages.length; i++) {
      ...
      const stageRow = (await query(
        `INSERT INTO pipeline_stages (pipeline_id, key, label, position)
         VALUES ($1, $2, $3, $4)
         RETURNING external_id, source_id, label`,
...
export async function deletePipeline(siteId, pipelineExternalId) {
  const result = await query(
    `DELETE FROM pipelines WHERE site_id = $1 AND (external_id::text = $2 OR source_id = $2)`,
