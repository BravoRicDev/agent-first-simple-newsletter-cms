...
const TOOL_META = {
  ...
  // ── Bug reports ───────────────────────────────────────────────────────
  "GET /api/agent/bug-reports": {
    name: "get_bug_reports",
    description: { en: "Admin only. Lists all bug reports across all users, newest first (filter by status/priority).", it: "Solo admin. Elenca tutte le segnalazioni bug di tutti gli utenti, più recenti prima (filtro per status/priority)." },
    inputSchema: {
  ...
  "GET /api/agent/bug-reports/mine": {
    name: "get_my_bug_reports",
    description: { en: "Lists ONLY the bug reports opened by the authenticated account — no admin permission needed. Use it to check whether a previously opened report has been taken in charge or resolved (note_sviluppatore explains what was done).", it: "Elenca SOLO le segnalazioni aperte dall'account autenticato — nessun permesso admin richiesto. Usalo per controllare se una segnalazione aperta in precedenza è stata presa in carico o risolta (note_sviluppatore spiega cosa è stato fatto)." },
    inputSchema: { status: z.enum(["aperto", "in_lavorazione", "risolto", "chiuso"]).optional(), limit: z.number().int().min(1).max(200).optional(), offset: z.number().int().min(0).optional() },
  },
  "POST /api/agent/bug-reports": {
    name: "create_bug_report",
    description: { en: "Opens a bug report for the developer, who reads reports 1-2 times a week (not real-time) — every ticket must be self-sufficient. Before opening one, ALWAYS interview the user: ask which page/section, exact reproduction steps, expected vs actual behavior. Never open a ticket from a vague statement like 'it doesn't work' without digging deeper. If it's a UI issue (visual appearance, layout, a component that looks wrong), ALWAYS also prepare a standalone HTML mockup 'before' (how it looks/behaves now, per the user's description) and 'after' (how it should look/behave) in mockup_before_html/mockup_after_html — complete self-contained HTML with inline CSS, no external dependencies. Only 'description' is required.", it: "Apre una segnalazione per lo sviluppatore, che le legge 1-2 volte a settimana (non in tempo reale) — ogni ticket deve essere autosufficiente. Prima di aprirla, INTERVISTA sempre l'utente: chiedi pagina/sezione, passi esatti di riproduzione, comportamento atteso vs osservato. Non aprire mai un ticket da una frase vaga tipo 'non funziona' senza approfondire. Se è un problema di interfaccia (aspetto visivo, layout, componente che sembra sbagliato), prepara SEMPRE anche un mockup HTML autonomo 'prima' (come appare/si comporta ora, secondo la descrizione dell'utente) e 'dopo' (come dovrebbe apparire/comportarsi) in mockup_before_html/mockup_after_html — HTML completo e autonomo con CSS inline, nessuna dipendenza esterna. Solo 'description' è obbligatorio." },
    inputSchema: {
      categoria: z.string().optional(), description: z.string().min(1),
      browser_info: z.string().optional(), steps_to_reproduce: z.string().optional(),
      expected_behavior: z.string().optional(), actual_behavior: z.string().optional(),
      mockup_before_html: z.string().optional().describe("Self-contained HTML with inline CSS, UI bugs only"),
      mockup_after_html: z.string().optional().describe("Self-contained HTML with inline CSS, UI bugs only"),
    },
  },
  "PUT /api/agent/bug-reports/:id": {
    name: "update_bug_report",
    description: { en: "Admin only. Updates status/priority/developer note of a bug report. Changing status sends an email to whoever opened it, including the developer note if present.", it: "Solo admin. Aggiorna status/priority/nota sviluppatore di una segnalazione. Cambiare lo status invia un'email a chi l'ha aperta, inclusa la nota sviluppatore se presente." },
    inputSchema: {
      id: z.number(),
      status: z.enum(["aperto", "in_lavorazione", "risolto", "chiuso"]).optional(),
      priority: z.enum(["bassa", "normale", "alta", "critica"]).optional(),
  ...
  // ── Opportunities Stats ──────────────────────────────────────────
  "GET /api/agent/sites/:siteId/opportunities-stats/search": {
    name: "opportunities_search",
    description: { en: "Searches opportunities with optional query, limit and offset.", it: "Cerca opportunità con query opzionale, limit e offset." },
    inputSchema: { site_id: z.number(), q: z.string().optional(), limit: z.number().optional(), offset: z.number().optional() },
  },
  "GET /api/agent/sites/:siteId/opportunities-stats/revenue": {
    name: "opportunities_revenue_stats",
    description: { en: "Returns revenue statistics by status and by pipeline for opportunities.", it: "Restituisce le statistiche di revenue per status e per pipeline delle opportunità." },
    inputSchema: { site_id: z.number(), pipeline_id: z.number().optional() },
  },
  "GET /api/agent/sites/:siteId/opportunities-stats/conversion": {
    name: "opportunities_conversion_funnel",
    description: { en: "Returns the conversion funnel for a specific pipeline.", it: "Restituisce il funnel di conversione per una pipeline specifica." },
    inputSchema: { site_id: z.number(), pipeline_id: z.number() },
  },
  "GET /api/agent/sites/:siteId/opportunities-stats/vendor": {
    name: "opportunities_vendor_stats",
    description: { en: "Returns vendor statistics (setters/closers) with total, won and revenue.", it: "Restituisce le statistiche dei vendor (setter/closer) con total, won e revenue." },
    inputSchema: { site_id: z.number() },
  },
  "GET /api/agent/sites/:siteId/opportunities-stats/trend": {
    name: "opportunities_trend",
    description: { en: "Returns trend data for opportunities over a time period.", it: "Restituisce i dati di trend delle opportunità per un periodo di tempo." },
    inputSchema: { site_id: z.number(), granularity: z.string().optional(), days: z.number().optional() },
  },
  "PUT /api/agent/sites/:siteId/opportunities/:id/owner": {
    name: "opportunities_reassign_owner",
    description: { en: "Reassigns the owner of an opportunity to a vendor by name.", it: "Riassegna il proprietario di un'opportunità a un vendor per nome." },
    inputSchema: { site_id: z.number(), id: z.number(), vendor: z.string().min(1) },
  },
};
