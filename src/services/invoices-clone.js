...
// "Doppio id": source_id reale preferito all'UUID interno in output e in
...
// Risolve l'id pubblico (source_id reale, fallback UUID) del contatto a cui la
...
async function contactPublicId(contactInternalId) {
  ...
  const c = (await query(
    "SELECT external_id, source_id FROM contacts WHERE id = $1",
    [contactInternalId]
  )).rows[0];
  if (!c) return null;
  return (c.source_id && String(c.source_id).trim()) || c.external_id;
}
...
export async function listInvoices(siteId, { status = null, contactId = null, limit = 20, startAfterId = null }, locationId) {
  ...
  if (contactId) {
    // contactId può essere l'UUID interno O il source_id reale del contatto
    const contact = await query(
      "SELECT id FROM contacts WHERE site_id = $2 AND (external_id::text = $1 OR source_id = $1) LIMIT 1",
      [contactId, siteId]
    );
  ...
  if (startAfterId) {
    // Cursore site-scoped: accetta sia UUID interno sia source_id reale
    const prev = (await query(
      "SELECT id FROM invoices WHERE site_id = $1 AND (external_id::text = $2 OR source_id = $2) LIMIT 1",
      [siteId, startAfterId]
    )).rows[0];
    if (prev) {
      params.push(prev.id);
  ...
  const invoicesWithItems = await Promise.all(
    rows.map(async (inv) => {
      ...
      // Aggiungi contact_public_id per serializzazione (source_id reale, fallback UUID)
