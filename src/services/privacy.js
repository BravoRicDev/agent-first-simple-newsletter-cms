...
export async function eraseContactData(siteId, email) {
  ...
  // source_id del contatto prima della cancellazione, per il push delete al CRM
  // sorgente (l'external_id locale è un UUID, non l'id sorgente).
  let sourceId = "";
  try {
    sourceId = (await query(
      "SELECT source_id FROM contacts WHERE site_id = $1 AND LOWER(email) = LOWER($2) LIMIT 1",
      [siteId, normalized]
    )).rows[0]?.source_id || "";
  } catch { /* best-effort */ }
  try {
