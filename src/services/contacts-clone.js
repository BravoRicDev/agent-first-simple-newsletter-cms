...
// (stile sorgente, camelCase) sulla colonna reale QUALIFICATA della query (alias
...
// Costruttori di condizione per /contacts/search (POST body.filters, array
// stile sorgente [{ field, operator, value }]). Chiave = "field:operator": SOLO
// le combinazioni esplicitamente presenti qui sono supportate — niente
// fallback generico che assembla SQL da field/operator arbitrari (rischio
// sia di injection sia, più semplice da centrare per sbaglio, di query
// con tipi incompatibili: es. confrontare c.id, integer, con una stringa
// avrebbe fatto fallire l'INTERA ricerca con un errore Postgres invece di
// limitarsi a ignorare il filtro non supportato). Una combinazione non
// presente qui viene semplicemente ignorata, stessa politica già usata da
// buildOrderBy sopra per i campi di sort non riconosciuti.
const FILTER_BUILDERS = {
  "email:eq": {
    joinRequired: false,
    sql: (ph) => `LOWER(c.email) = LOWER(${ph})`,
    normalize: (v) => normalizeEmail(v),
  },
  // phone/firstName sono salvati come "custom values" di profilo (stessa
  // tabella/colonna JSONB usata da createContact/updateContact per
  // firstName/lastName/phone/ecc., vedi contact_custom_values più sotto).
  "phone:eq": {
    joinRequired: true,
    sql: (ph) => `(cv.values->>'phone') = ${ph}`,
    normalize: (v) => v,
  },
  "firstNameLowerCase:contains": {
    joinRequired: true,
    // Autoritativo: il raw sorgente persistito dal sync (db/130), quando
    // presente. Fallback sul firstName locale (contact_custom_values) per
    // i contatti creati/aggiornati via clone-API e non ancora ri-passati
    // dal sync, o mai sincronizzati da sorgente.
    sql: (ph) =>
      `(LOWER(c.source_contact_raw->>'firstNameLowerCase') ILIKE '%' || LOWER(${ph}) || '%' ` +
      `OR LOWER(cv.values->>'firstName') ILIKE '%' || LOWER(${ph}) || '%')`,
    normalize: (v) => v,
  },
};

// Costruisce l'ORDER BY dall'array sort stile sorgente [{ field, direction }].
...
// Costruisce le clausole WHERE aggiuntive dall'array filters stile sorgente
// [{ field, operator, value }] usando FILTER_BUILDERS (sopra). Placeholder
// $N a partire da startIdx: il chiamante deve poi accodare `params` al
// proprio array e proseguire il conteggio da `nextIdx`, così l'intera
// query (filtri + query/tag/email + paginazione) usa UNA sola sequenza di
// indici coerente con l'array parametri — niente riordino a posteriori.
function buildFilterWhere(filters, startIdx) {
  if (!Array.isArray(filters) || filters.length === 0) {
    return { whereSql: "", joinSql: "", params: [], nextIdx: startIdx };
  }
  const clauses = [];
  const params = [];
  let joinSql = "";
  ...
  for (const f of filters) {
    ...
    const builder = FILTER_BUILDERS[`${field}:${operator}`];
    if (!builder) continue; // combinazione non supportata: filtro ignorato, non un errore

    if (builder.joinRequired && !joinSql) {
      joinSql = "LEFT JOIN contact_custom_values cv ON cv.contact_id = c.id AND cv.object_key = 'contact' AND cv.site_id = c.site_id";
    }

    const ph = `$${idx}`;
    clauses.push(builder.sql(ph));
    params.push(builder.normalize(value));
    idx++;
  }

  return { whereSql: clauses.join(" AND "), joinSql, params, nextIdx: idx };
}

// Chiavi che NON sono custom field sorgente: profilo + campi top-level del contatto.
// Vanno escluse dall'array customFields (in sorgente sono campi separati, non custom).
...
// AND (source_id=$2 OR external_id::text=$2 OR field_key=$2), ritorna field_key
// se trovato altrimenti null.
async function resolveCustomFieldKey(siteId, idOrKey) {
  const rows = (await query(
    "SELECT field_key FROM custom_fields WHERE site_id = $1 AND object_key = 'contact' AND active = true AND (source_id = $2 OR external_id::text = $2 OR field_key = $2)",
    [siteId, idOrKey]
  )).rows;
  return rows.length > 0 ? rows[0].field_key : null;
}

// Costruisce customFields nello SHAPE REALE di sorgente: [{ id, value }] dove id è il
// source_id VERO del custom field (non il nostro UUID external_id — era il bug: si
// usava external_id e si aggiungevano key/field_value, campi che in sorgente non
// esistono). Per un campo sincronizzato source_id è sempre valorizzato; fallback a
// external_id solo per definizioni locali mai sincronizzate (assenti in sorgente).
function buildCustomFields(customValues = {}, customFieldDefs = []) {
  ...
  for (const [k, v] of Object.entries(customValues || {})) {
    ...
    const def = map.get(k);
    if (!def) continue;
    out.push({ id: def.source_id || def.external_id, value: v });
  }
  return out;
}

// row.source_contact_raw è JSONB (pg lo restituisce già oggetto); tollera anche
...
// Stringa: null se vuota/assente (sorgente usa null, non "").
function nn(v) { return v === "" || v === undefined || v === null ? null : v; }
// Prendi dal raw (autoritativo, mirror sorgente) se la chiave c'è; altrimenti fallback.
...
export async function serializeContact(row, customValues = {}, customFieldDefs = []) {
  ...
  // Custom field nello shape reale di sorgente: [{ id: <source_id>, value }].
...
// contatto nella risposta reale di sorgente a questo endpoint (vedi
// .tmp-compare/source-real-francesco.json). NON tocca serializeContact (usato da
// GET /contacts e GET /contacts/:id, che hanno un altro shape). Campi letti dal
// raw sorgente persistito dal sync (db/130) quando presente, con fallback sensati;
// null dove sorgente usa null (non stringa vuota); opportunities/followers/
// attributionSource/additionalEmails/... dal raw (già nel payload di
// /contacts/search, nessuna chiamata extra). customFields con il source_id reale.
export async function serializeContactSearch(row, customValues = {}, customFieldDefs = []) {
  if (!row) return null;
  const generatedExtId = await getExternalId("contacts", row.id);
  const id = publicId(row) || generatedExtId;
  const raw = parseRaw(row.source_contact_raw);
...
export async function getContactCustomFields(siteId) {
  const rows = (await query(
    "SELECT id, field_key, external_id, source_id FROM custom_fields WHERE site_id = $1 AND object_key = 'contact' AND active = true",
    [siteId]
  )).rows;
  return rows;
}
...
export async function createContact(siteId, data = {}) {
  ...
  // 1) Formato sorgente reale: { id: "<source_id>", value: "..." } — qui id è il source_id vero
  // 2) Formato interno legacy: { key: "<field_key>", field_value: "..." }
  // Risolviamo sempre tramite resolveCustomFieldKey che accetta sia source_id che external_id che field_key.
  if (Array.isArray(data.customFields)) {
    for (const cf of data.customFields) {
      let fieldKey;
      if (cf.id) {
        // Formato sorgente/esterno: usa id (può essere source_id o external_id/UUID interno)
...
export async function listContacts(siteId, filters = {}, serialize = serializeContact) {
  ...
  // filters array da POST /contacts/search [{ field, operator, value }].
  const filterArray = Array.isArray(filters.filters) ? filters.filters : null;

  let whereClause = "c.site_id = $1";
  const params = [siteId];
  let paramIdx = 2;

  // Filtri array applicati PRIMA di query/tag/email: un'unica sequenza di
  // placeholder $N coerente con params, nessun riordino dell'array dopo.
  const { whereSql: filterWhereSql, joinSql: filterJoinSql, params: filterParams, nextIdx } = buildFilterWhere(filterArray, paramIdx);
  if (filterWhereSql) {
    whereClause += ` AND ${filterWhereSql}`;
  ...
  const countRes = (await query(
    `SELECT COUNT(*) as total FROM contacts c${filterJoinSql ? " " + filterJoinSql : ""} WHERE ${whereClause}`,
    params
  )).rows[0];
  ...
  const querySql = `
    ...
    FROM contacts c${filterJoinSql ? " " + filterJoinSql : ""}
...
export async function searchContacts(siteId, filters = {}) {
  // Serializer DEDICATO: /contacts/search deve essere byte-identico a sorgente
...
export async function upsertContact(siteId, data = {}) {
  ...
  const existing = (await query("SELECT id, source_id, external_id FROM contacts WHERE site_id = $1 AND email = $2", [siteId, email])).rows[0];
...
export async function serializeNote(row) {
  ...
  let userId = null;
  if (row.user_id) {
    const userRow = (await query("SELECT source_id, external_id FROM users WHERE id = $1", [row.user_id])).rows[0];
    userId = publicId(userRow) || (await getExternalId("users", row.user_id));
  }
  let contactId = null;
  if (row.contact_id) {
    const contactRow = (await query("SELECT source_id, external_id FROM contacts WHERE id = $1", [row.contact_id])).rows[0];
...
export async function getContactFollowers(siteId, contactExternalId) {
  ...
  const rows = (await query(
    `SELECT u.external_id, u.source_id, u.name, u.email FROM contact_followers cf
...
export async function getContactAppointments(siteId, contactExternalId) {
  ...
  const rows = (await query(
    `SELECT external_id, source_id, title, start_time, end_time, status
...
// AGGIUNGE tag esistenti alla lista (non sovrascrive). Restituisce
// tutti i tag dopo l'aggiunta.

export async function addContactTags(siteId, contactExternalId, newTags) {
  const contact = await findByAnyId("contacts", siteId, contactExternalId);
  if (!contact || contact.site_id !== siteId) {
    const err = new Error("Contatto non trovato");
    err.status = 404;
  ...
  await query(
    "UPDATE contacts SET tags = $1, updated_at = NOW() WHERE id = $2 AND site_id = $3",
    [merged, contact.id, siteId]
  );

  const refreshed = (await query(
    "SELECT tags FROM contacts WHERE id = $1 AND site_id = $2",
    [contact.id, siteId]
  )).rows[0];
  return { tags: refreshed?.tags || merged };
}

export async function removeContactTags(siteId, contactExternalId, tagsToRemove) {
  const contact = await findByAnyId("contacts", siteId, contactExternalId);
  if (!contact || contact.site_id !== siteId) {
    const err = new Error("Contatto non trovato");
    err.status = 404;
  ...
  const existing = Array.isArray(contact.tags) ? contact.tags : [];
  const remaining = existing.filter((t) => !tagsToRemove.includes(t));

  await query(
    "UPDATE contacts SET tags = $1, updated_at = NOW() WHERE id = $2 AND site_id = $3",
    [remaining, contact.id, siteId]
  );
  ...
  return { tags: refreshed?.tags || remaining };
}
