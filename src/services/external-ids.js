...
export const WHITELIST_TABLES = {
  ...
  // id source_id — db/120_source_id_columns.sql / db/124_source_sync_new_modules.sql).
  // Prefisso "source_" per non collidere con le tabelle omonime del CMS (es.
  // `workflows`, il motore Automazioni v2 nativo — tutt'altra cosa).
  source_workflows: true,
  source_funnels: true,
  source_custom_values: true,
};
...
// (external_id) SIA l'id reale del CRM sorgente (source_id), sempre scoped per
// site_id (necessario: due siti gemelli sullo stesso account sorgente possono
// condividere lo stesso source_id — vedi db/126_source_id_per_site.sql).
export async function findByAnyId(table, siteId, idValue) {
  ...
  const row = (await query(
    `SELECT * FROM ${table} WHERE site_id = $1 AND (external_id::text = $2 OR source_id = $2) LIMIT 1`,
    [siteId, idValue]
  )).rows[0];
  return row || null;
}

// "id" pubblico da esporre nelle risposte clone-API: preferisce il source_id
// reale, ricade sull'UUID interno per record mai sincronizzati da/verso sorgente.
export function publicId(row) {
  if (!row) return null;
  return (row.source_id && String(row.source_id).trim()) || row.external_id || null;
}
