...
// ── LOCATIONS (Sites) ────────────────────────────────────────────────────
// Parità sorgente (round 15): l'id pubblico di una location è il SUO
// location_external_id (per sorgente il "location id" È l'identificativo della
// location, 20 char alfanumerici), non l'UUID interno del site. Le tabelle
// sites NON hanno colonna source_id → niente findByAnyId: risoluzione dedicata
...
export async function getLocationByIdOrExternalId(identifier) {
  ...
  // (quello che un'automazione n8n ha già in mano da sorgente). La colonna è
  ...
  // Round 19 — parità col GET /locations/{id} di sorgente: se il source-sync
  // "location-info" ha catturato il payload REALE della location (tabella
  // source_location_info.raw = risposta integrale del sorgente, inclusi i
  ...
  // Un'automazione che leggeva il JSON di sorgente continua a funzionare
  ...
  // esiste in sorgente): dopo il próximo giro di sync il raw torna autorità.
  const info = (await query(
    "SELECT raw FROM source_location_info WHERE site_id = $1",
    [row.id]
  )).rows[0];
...
export async function createUser({ siteId, firstName, lastName, email, roles }) {
  ...
  const result = await query(
    `INSERT INTO users (email, name, role, site_id, status)
     ...
     RETURNING id, external_id, source_id, email, name, role, created_at`,
