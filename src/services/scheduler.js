...
async function schedulerTick({ webhookAllowPrivate = false } = {}) {
  ...
  try {
    ...
    // Push bidirezionale verso il CRM sorgente, opzionale per
    // sito: CANALE single-fire (advisory lock per-sito + anti-echo + no-import
    // + replica al passo, vedi services/source-sync/push.js).
    try {
      const { runSourcePushDue } = await import("./source-sync/push.js");
      const gp = await runSourcePushDue();
      if (gp.started > 0) logger.info(`sorgente push: avviati ${gp.started} run`);
    } catch (err) {
      logger.error(`sorgente push tick fallito: ${err.message}`);
    }
  } finally {
    if (lockClient) {
      try {
