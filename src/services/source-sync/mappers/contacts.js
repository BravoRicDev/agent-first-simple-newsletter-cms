...
async function upsertContact(ctx, extId, contact) {
  ...
  try {
    ...
    // source_id NON è quello sorgente che stiamo per scrivere (doppio id: vedi
    // db/120_source_id_columns.sql — external_id resta SEMPRE l'id locale
    // dell'oggetto CMS, mai scritto/letto dal source-sync; source_id è l'id
    // esatto della risorsa su CRM sorgente, stringa non-uuid). Un record
    // creato dal CMS ha source_id = '' (default): l'adozione prevale quando
    // l'id sorgente locale non coincide col sorgente, sovrascrivendolo con
    // quello reale. Se source_id === extId il record è già nostro: si
    // prosegue verso l'upsert normale (skip-if-unchanged/update).
    const shouldAdopt = existing && existing.source_id !== extId;
    const cols = {
      email,
    ...
    if (shouldAdopt) {
      const upd = (
        await query(
          `UPDATE contacts SET source_id = $1, email = $2, status = $3, notes = $4, tags = $5::text[], updated_at = $6 WHERE id = $7 RETURNING *`,
...
async function storeProfiles(ctx, contactId, contact) {
  ...
  if (contact.customFields && Array.isArray(contact.customFields)) {
    for (const cf of contact.customFields) {
      ...
      // locale già salvato da mappers/custom-fields.js (source_id, non external_id).
      let fieldKey = cf.key || cf.field_key;
      if (!fieldKey && cf.id) {
        const def = (await query(
          "SELECT field_key FROM custom_fields WHERE source_id = $1 AND site_id = $2 LIMIT 1",
          [cf.id, siteId]
        )).rows[0];
...
// Soglia di conferma per lo stop anticipato: più di 25 contatti consecutivi
// già sincronizzati in ordine dateUpdated decrescente (non solo 1, per
// margine di sicurezza contro casi limite di ordinamento/tie-break).
// Counta contatti individuali, non pagine: con pageLimit=100, 25 contatti
// possono stare su 1 pagina o disperdersi su più pagine a seconda dell'ordinamento.
...
// CONTATTO su sorgente NON cambia quando si aggiunge/modifica una sotto-risorsa
        ...
        for (const c of pageContacts) {
          try {
            ...
            if (row && row.id) {
              ...
              if (!dryRun) {
                await query(
                  "UPDATE contacts SET source_contact_raw = $1 WHERE id = $2",
...
export async function syncForContacts(ctx, extIds) {
  ...
  for (const extId of extIds) {
    ...
    try {
      ...
      // campo "contactEmail" sul payload nota — non esiste su sorgente reale, quindi
      ...
      const noteContactRow = (await query(
        "SELECT id, email FROM contacts WHERE source_id=$1 AND site_id=$2",
        [extId, siteId]
      )).rows[0];
