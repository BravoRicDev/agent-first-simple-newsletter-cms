...
// Tabella source_workflows: NON confondere con `workflows`, il motore
// "Automazioni v2" nativo del CMS (src/services/workflows.js) — due sistemi
// distinti, nome mapper "source-workflows" scelto apposta per non collidere.
// ─────────────────────────────────────────────────────────────────────────
...
export async function syncAll(ctx) {
  ...
  try {
    ...
    addStat("source-workflows", "fetched", items.length);
    ...
    for (const w of items) {
      try {
        ...
        if (!externalId) {
          ...
          log(`source-workflow saltato: id mancante (${JSON.stringify(w).slice(0, 200)})`);
          continue;
        }
        ...
        if (dryRun) {
          addStat("source-workflows", "upserted", 1);
          continue;
        }

        const { action } = await upsertByExternalId({
          table: "source_workflows",
          siteId,
          externalId,
          cols,
          timestamps,
        ...
        if (action === "inserted") addStat("source-workflows", "upserted", 1);
        else if (action === "updated") addStat("source-workflows", "updated", 1);
        else addStat("source-workflows", "skipped", 1);
      } catch (err) {
        addStat("source-workflows", "errors", 1);
        log(`source-workflow ${w.id}: ${err.message}`);
      }
    }
  } catch (err) {
    addStat("source-workflows", "errors", 1);
    log(`syncAll source-workflows fallito: ${err.message}`);
    throw err;
  }
}
