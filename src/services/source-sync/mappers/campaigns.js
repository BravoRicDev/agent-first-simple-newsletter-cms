    ...
    if (!dryRun) {
      for (const campaign of campaigns) {
        try {
          ...
          const existing = (
            await query(
              "SELECT id FROM newsletter_campaigns WHERE source_id = $1 AND site_id = $2 LIMIT 1",
              [campaign.id, siteId]
            )
          ).rows[0];
          ...
          if (!existing) {
            row = (
              await query(
                `INSERT INTO newsletter_campaigns (site_id, source_id, subject, html_content, status, created_at)
