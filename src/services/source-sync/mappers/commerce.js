        ...
        try {
          ...
          for (const item of items) {
            try {
              ...
              // Scopato per invoice_id (db/126_source_id_per_site.sql:
              // UNIQUE(invoice_id, source_id), invoice_items non ha site_id
              // proprio): stesso bug di pipeline_stages, vedi upsert.js —
              // due siti sullo stesso account sorgente con lo stesso source_id
              // articolo altrimenti si "rubano" la riga a vicenda.
              const existing = (
                await query(
                  "SELECT id FROM invoice_items WHERE source_id = $1 AND invoice_id = $2 LIMIT 1",
                  [itemId, row?.id || null]
                )
              ).rows[0];
              ...
              if (!existing && !dryRun) {
                ...
                await query(
                  `INSERT INTO invoice_items (source_id, invoice_id, description, quantity, unit_price, total, created_at)
