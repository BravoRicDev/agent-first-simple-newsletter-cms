      ...
      try {
        ...
        if (!dryRun && cal.teamMembers && Array.isArray(cal.teamMembers)) {
          const calendarId = (await query(
            "SELECT id FROM calendars WHERE source_id=$1 AND site_id=$2",
            [cal.id, siteId]
          )).rows[0]?.id;

          if (calendarId) {
            // Risolvi utenti per source_id
    ...
    for (const contactExtId of extIds) {
      try {
        ...
        for (const apt of apts) {
          try {
            // Risolvi calendar_id da calendarId (sorgente) → calendars.source_id → id
            let calendarId = null;
            if (apt.calendarId) {
            ...
            // sorgente espone lo stato come "appointmentStatus", non "status" (che
