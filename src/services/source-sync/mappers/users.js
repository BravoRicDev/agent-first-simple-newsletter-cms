  ...
  for (const u of users) {
    try {
      ...
      if (err.message?.includes("duplicate key") && err.message?.includes("email")) {
        ...
        // cerca per (source_id, site_id=<questo sito>) e non trova la riga
        ...
        const existing = (
          await query("SELECT id, source_id FROM users WHERE email = $1 LIMIT 1", [u.email])
        ).rows[0];
        if (existing) {
          // S1 v2: adotta il record locale esistente (stessa email) quando il
          // suo source_id NON è quello sorgente (doppio id: db/120_source_id_columns.sql
          // — external_id resta l'id locale, mai toccato dal source-sync).
          if (existing.source_id !== u.id) {
            if (!dryRun) {
              await query("UPDATE users SET source_id = $1 WHERE id = $2", [u.id, existing.id]);
            }
