      ...
      for (const sub of submissions) {
        try {
          const existing = !dryRun
            ? (
                await query(
                  "SELECT id, created_at FROM form_submissions WHERE source_id = $1 AND site_id = $2 LIMIT 1",
                  [sub.id, siteId]
                )
              ).rows[0]
            : null;
          ...
          if (existing) {
            ...
            if (!dryRun) {
              await query(
                `INSERT INTO form_submissions (site_id, source_id, form_slug, data, created_at)
                 VALUES ($1, $2, $3, $4, $5)`,
