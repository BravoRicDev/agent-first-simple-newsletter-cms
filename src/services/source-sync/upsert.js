...
//
// DOPPIO ID (db/120_source_id_columns.sql): il parametro `externalId` qui sotto
// è l'ID ESATTO della risorsa sul CRM sorgente (stile CRM sorgente: stringa
// alfanumerica di 20 caratteri, es. "eMjqNVexkS7CyIM2qdtg") e viene
// scritto/letto sulla colonna `source_id` (VARCHAR, default ''), MAI su
 ...
 *                              nella colonna source_id, MAI in external_id —
 ...
 *   source_id, es. "pipeline_id" — deve rispecchiare l'indice UNIQUE(parent,
 *   source_id) di db/126_source_id_per_site.sql. Ignorato se la tabella ha
 *   site_id proprio.
...
export async function upsertByExternalId({ table, siteId, externalId, cols, timestamps = {}, scopeCol = null, scopeValue = null }) {
  ...
  // (pipeline_id/invoice_id). db/126_source_id_per_site.sql ha reso l'indice
  // source_id di QUESTE tabelle UNIQUE(parent_id, source_id) — NON più globale —
  // proprio per permettere a due siti sullo stesso account sorgente (stessi
  // source_id sorgente) di avere ciascuno le proprie righe figlie. Se la
  // lookup qui sotto non scopa allo stesso modo (parent_id, non solo
  // source_id), due siti gemelli con lo stesso source_id finiscono per
  ...
  const existing = (
    await query(
      hasSiteId
        ? `SELECT ${selectCols.join(", ")} FROM ${table} WHERE source_id = $1 AND site_id = $2 LIMIT 1`
        : hasScope
        ? `SELECT ${selectCols.join(", ")} FROM ${table} WHERE source_id = $1 AND ${scopeCol} = $2 LIMIT 1`
        : `SELECT ${selectCols.join(", ")} FROM ${table} WHERE source_id = $1 LIMIT 1`,
...
/** Risolve un id sorgente (source_id, doppio id) in id interno della tabella (scope sito). */
export async function findInternalId(table, siteId, externalId) {
  ...
  const r = await query(
    `SELECT id FROM ${table} WHERE source_id = $1 AND site_id = $2 LIMIT 1`,
    [externalId, siteId]
  );
