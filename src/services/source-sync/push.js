...
// ─────────────────────────────────────────────────────────────────────────
// Push bidirezionale verso il CRM sorgente — CMS → sorgente.
//
...
//   4. ANTI-ECHO: le mutate con origine 'source_in'/'import' NON vengono mai
//      rispedite a sorgente (evita cascate A→sorgente→A→sorgente…).
//
...
const NO_ECHO_ORIGINS = new Set(["source_in", "import"]);
...
/**
 * Accoda una mutata da propagare a sorgente. Idempotente/aggressiva-dedup:
...
export async function enqueuePush(siteId, entityType, opts = {}) {
  ...
  // ANTI-ECHO: nulla che arriva da sorgente/import torna a sorgente.
...
async function loadContactForPush(siteId, id) {
  ...
  // Mappa field_key → source_id (id definizione sorgente, doppio id: vedi
  // db/120_source_id_columns.sql) caricata UNA volta: prima ogni chiave custom
  // faceva partire una query dedicata (N+1).
  const fieldDefs = (
    await query(
      "SELECT field_key, source_id FROM custom_fields WHERE site_id = $1 AND object_key = 'contact' AND source_id <> ''",
      [siteId]
    )
  ).rows;
  const externalIdByKey = new Map(fieldDefs.map((r) => [r.field_key, r.source_id]));
...
async function loadOpportunityForPush(siteId, id) {
  const row = (
    await query(
      `SELECT o.*, p.source_id AS pipeline_ext_id
  ...
  if (row.pipeline_id && row.stage) {
    const st = (
      await query(
        "SELECT source_id FROM pipeline_stages WHERE pipeline_id = $1 AND key = $2 AND source_id <> '' LIMIT 1",
        [row.pipeline_id, row.stage]
      )
    ).rows[0];
    pipelineStageExtId = st?.source_id || "";
  }

  const contact =
    (
      await query(
        "SELECT id, source_id FROM contacts WHERE site_id = $1 AND LOWER(email) = LOWER($2) LIMIT 1",
  ...
  if (row.owner_id) {
    const u = (
      await query("SELECT source_id FROM users WHERE id = $1 AND source_id <> '' LIMIT 1", [row.owner_id])
    ).rows[0];
    ownerExtId = u?.source_id || "";
  }
  ...
  return {
    ...
    contactSourceId: contact?.source_id || "",
...
async function sendContact(client, payload) {
  ...
  const sourceId = data?.contact?.id || data?.id || "";
  return { sourceId };
}
...
async function sendOpportunity(client, payload) {
  ...
  if (sourceId) {
    // UPDATE opportunità esistente in sorgente.
    const body = {
    ...
    await client.write("PUT", `/opportunities/${sourceId}`, body);
    return { sourceId };
  }
  // CREATE: sorgente richiede l'id sorgente del contatto.
  if (!payload.contactSourceId) {
    throw new Error("Impossibile creare opportunità su sorgente: contatto non ancora sincronizzato (manca contactId sorgente)");
  }
  const body = {
    contactId: payload.contactSourceId,
  ...
  for (const sid of sites) {
    ...
    try {
      ...
      for (const row of rows) {
        summary.processed++;
        try {
          let sourceId = row.external_id || "";
          // Per delete, risolvi l'id sorgente anche dall'entità se la coda non lo ha.
          if (row.operation === "delete" || !sourceId) {
            const entity = row.entity_type === "contact"
              ? (await query("SELECT source_id FROM contacts WHERE id=$1 AND site_id=$2", [row.entity_id, sid])).rows[0]
              : (await query("SELECT source_id FROM opportunities WHERE id=$1 AND site_id=$2", [row.entity_id, sid])).rows[0];
            sourceId = (entity?.source_id || row.external_id || "") || "";
          }

          if (row.operation === "delete") {
            await sendDelete(client, row.entity_type, sourceId);
            await query(
          ...
          if (row.entity_type === "contact") {
            ...
            sourceId = res.sourceId || sourceId;
            if (sourceId) {
              await query(
                `UPDATE contacts SET source_id = $1, updated_at = NOW()
                 WHERE id = $2 AND site_id = $3`,
                [sourceId, row.entity_id, sid]
              ).catch(() => {});
            }
            ...
            if (sourceId) {
              await query(
                `UPDATE opportunities SET source_id = $1, updated_at = NOW()
                 WHERE id = $2 AND site_id = $3`,
                [sourceId, row.entity_id, sid]
              ).catch(() => {});
            }
      ...
      if (locked) {
        try {
          ...
          logger.error(`source-push: unlock fallito (site ${sid}): ${err.message}`);
        }
      }
      lockClient.release();
    }
...
export async function runSourcePushDue() {
  const due = (
    await query(
      `SELECT q.site_id
  ...
  for (const row of due) {
    ...
    processPushQueue({ siteId })
      .catch((err) => logger.error(`source-push cron site ${siteId}: ${err.message}`))
