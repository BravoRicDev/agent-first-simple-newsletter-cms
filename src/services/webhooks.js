...
// es. verso CRM sorgente). `origin` traccia da dove nasce l'evento
// ('cms'|'agent'|'source_in'|'import') per l'anti-echo del push sorgente.
//
...
// `options.origin` ('cms'|'agent'|'source_in'|'import') registra da dove
...
async function runInboundAction(siteId, eventType, rule, body, webhookId) {
  ...
  try {
    switch (action) {
      case "create_contact": {
        if (!email) return 0;
        await upsertContact(siteId, email, { origin: "source_in" });
        const tags = Array.isArray(config.tags) ? config.tags : (Array.isArray(body.tags) ? body.tags : []);
        for (const tag of tags) await addContactTag(siteId, email, tag, { origin: "source_in" });
        return 1;
      }
      case "emit_event": {
        if (!email) return 0;
      ...
      case "add_tag": {
        ...
        await addContactTag(siteId, email, config.tag, { origin: "source_in" });
        return 1;
      }
      case "create_task": {
