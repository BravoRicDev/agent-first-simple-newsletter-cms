...
//
// Parity source_id: id survey/submission/contatto collegato preferiscono il
// source_id reale del CRM sorgente quando presente, con fallback all'UUID
// interno — stesso pattern già applicato a contatti/opportunità/calendari/
// conversazioni/tag/custom field/form. survey_questions NON ha source_id (mai
// sincronizzata da sorgente come risorsa a sé, è il modello normalizzato interno
...
async function serializeSubmission(row, locationId) {
  ...
  const surveyRow = (await query(
    "SELECT source_id, external_id FROM surveys WHERE id = $1",
    [row.survey_id]
  )).rows[0];
...
export async function createSurveySubmission(siteId, surveyRef, { email, contactId, answers = {} }, locationId) {
  ...
  // id esterno (uuid o source_id reale): gestiamo entrambi per robustezza.
  let found;
  ...
  if (contactId) {
    // contactId è un id esterno (uuid o source_id reale): risolvilo all'id interno
