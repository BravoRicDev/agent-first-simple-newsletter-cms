...
//
// Parity source_id: id form/submission/contatto collegato preferiscono il
...
export function serializeSubmission(row, locationId = null) {
  ...
  return {
    id: publicId(row),
    formId: publicId({ source_id: row.form_source_id, external_id: row.form_id_ext }),
    contactId: publicId({ source_id: row.contact_source_id, external_id: row.contact_id_ext }),
...
// Risolvi formId (UUID o source_id reale) → form id interno.
...
export async function listSubmissions(siteId, filters = {}, locationId = null) {
  ...
  // Filtro formIds (CSV di uuid o source_id)
  ...
  const rows = (await query(
    `SELECT fs.*,
            f.external_id AS form_id_ext, f.source_id AS form_source_id,
            c.external_id AS contact_id_ext, c.source_id AS contact_source_id
