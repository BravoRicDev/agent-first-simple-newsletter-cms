...
// Hook per il PUSH opzionale verso il CRM sorgente.
// Fire-and-forget; no-op se il push non è abilitato per il sito. `origin`
// implementa l'anti-echo (le mutate arrivate da sorgente non vengono rispedite).
...
export async function deleteOpportunity(siteId, id) {
  ...
  if (!row) return 0;
  // IMPORTANTE: l'id da cancellare su sorgente è il `source_id` (id sul CRM sorgente),
  // NON `external_id` (UUID locale assegnato dalla migrazione 090).
  pushOpportunity(siteId, row.id, { operation: "delete", externalId: String(row.source_id || "") });
