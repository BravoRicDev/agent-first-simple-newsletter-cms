...
async function serializeEvent(row, locationId) {
  ...
  if (row.calendar_id) {
    const calRow = await query(
      "SELECT source_id, external_id FROM calendars WHERE id = $1",
      [row.calendar_id]
    );
    if (calRow.rows[0]) {
  ...
  // Resolve contactId to sorgente uuid if present
  let contactId = row.contact_id || null;
  if (contactId) {
    const cRow = await query(
      "SELECT source_id, external_id FROM contacts WHERE id = $1",
      [contactId]
    );
    if (cRow.rows[0]) {
      contactId = publicId(cRow.rows[0]) || cRow.rows[0].external_id || null;
    }
  }

  return {
    id,
    eventId: id, // alias per backward compat con client esistenti
    calendarId,
    locationId,
    contactId,
    title: row.title,
    appointmentStatus: row.appointment_status || mapLegacyStatus(row.status),
    status: row.appointment_status || mapLegacyStatus(row.status), // alias per backward compat
    assignedUserId: row.assigned_user_id || null,
    meetingLocationType: "default",
    address: row.contact_email || null,
...
export async function createCalendar(siteId, input, locationId) {
  ...
  // team members opzionali (uuid user o source_id)
...
export async function getCalendar(siteId, externalId, locationId) {
  ...
  const members = await query(
    `SELECT u.source_id, u.external_id FROM calendar_members cm
...
export async function createAppointment(siteId, input, locationId) {
  const { calendarId, title, startTime, endTime, email, contactId: inputContactId } = input;
  ...
  // Risolvi email del contatto: priorità a inputContactId (sorgente uuid), poi email
  let resolvedEmail = email || null;
  if (inputContactId) {
    const cRow = await findByAnyId("contacts", siteId, inputContactId);
    if (cRow) resolvedEmail = cRow.email || resolvedEmail;
  }

  // upsert contatto per email (come fa booking.js)
  let contactIdInt = null;
  if (resolvedEmail) {
    const contactRow = await query("SELECT id FROM contacts WHERE site_id = $1 AND email = $2", [siteId, resolvedEmail]);
  ...
  const apptStatus = input.appointmentStatus || "confirmed";
  const legacyStatus = mapTargetStatusToLegacy(apptStatus);
  ...
  const result = await query(
    `INSERT INTO booking_appointments (
     ...
     ) VALUES ($1, $2, '', $3, '', $4, $5, $6, $7, $8, $9, 'UTC', NOW(), NOW())
     RETURNING *`,
    [siteId, calendarIdInt, resolvedEmail || "", title, desc, startTime, endTimeVal, legacyStatus, apptStatus]
  );
...
export async function updateAppointment(siteId, eventId, input, locationId) {
  ...
  const newStatus = input.appointmentStatus !== undefined ? input.appointmentStatus : input.status;
  if (newStatus !== undefined) {
    const legacyStatus = mapTargetStatusToLegacy(newStatus);
    updates.status = legacyStatus;
    updates.appointment_status = newStatus;
  }
...
export async function getFreeSlots(siteId, calendarId, startDate, endDate, locationId) {
  ...
  // Raggruppa per data nel formato sorgente: { "YYYY-MM-DD": { slots: ["ISO-string", ...] } }
  const result = {};
  for (const slot of slots) {
    ...
    result[dateKey].slots.push(slot.start.toISOString());
  }
