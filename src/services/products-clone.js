...
// Serializzazione camelCase con "doppio id" (source_id reale preferito
...
export async function listProducts(siteId, { limit = 20, startAfterId = null }, locationId) {
  ...
  if (startAfterId) {
    // Il cursore può essere sia l'UUID interno sia il source_id reale: lookup
    // site-scoped (due siti gemelli possono condividere lo stesso source_id).
    const prev = (await query(
      "SELECT id FROM products WHERE site_id = $1 AND (external_id::text = $2 OR source_id = $2) LIMIT 1",
      [siteId, startAfterId]
    )).rows[0];
    if (prev) {
      params.push(prev.id);
