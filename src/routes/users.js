...
const userSchema = z.object({
  ...
  role: z.enum(["admin", "collaboratore"]),
...
router.get("/admin/users", requireAuth, authorize("users", "read"), async (req, res, next) => {
  try {
    ...
    if (req.user.role === "superadmin") {
      result = await query(
        `SELECT u.id, u.email, u.name, u.role, u.status, u.site_id, s.name AS site_name
...
router.post("/admin/users", requireAuth, authorize("users", "create"), async (req, res, next) => {
  try {
    ...
    const inserted = await query(
      "INSERT INTO users (email, name, role, site_id) VALUES ($1, $2, $3, $4) RETURNING id",
      [email, data.name, data.role, siteId]
    );
    await auditLog({
...
router.post("/admin/users/:id", requireAuth, authorize("users", "update"), async (req, res, next) => {
  try {
    ...
    const allowedRoles = req.user.role === "superadmin"
      ? ["admin", "collaboratore", "superadmin"]
      : ["admin", "collaboratore"];
    ...
    await query(
      `UPDATE users SET name = $1, role = $2, site_id = $3, status = $4, updated_at = NOW()${changed ? ", token_version = token_version + 1" : ""} WHERE id = $5`,
      [name || "", role, finalSiteId, safeStatus, targetId]
    );
    if (changed) {
      await auditLog({
