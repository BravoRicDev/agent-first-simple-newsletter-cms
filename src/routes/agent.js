...
import { listBugReports, listMyBugReports, createBugReport, updateBugReport } from "../services/bug-reports.js";
...
router.put("/api/agent/sites/:siteId/tracking", requireAuth, requireAgent, async (req, res, next) => {
  try {
    ...
    for (const f of [
      ...
      "consentBannerText", "consentAcceptLabel", "consentRejectLabel", "consentPrivacyUrl", "consentCookieHours",
...
import { registerSourcePushRoutes } from "./agent-source-push.js";
...
registerSourcePushRoutes(router);
...
// ── Segnalazioni bug ─────────────────────────────────────────────────────
// Stessa logica di src/routes/bug-reports.js (route umane/token generico),
// qui riesposta sotto /api/agent per farla scoprire automaticamente come
// tool MCP da discoverTools() in services/mcp-tools.js.

const agentBugReportCreateSchema = z.object({
...
const agentBugReportUpdateSchema = z.object({
...
router.get("/api/agent/bug-reports", requireAuth, requireAgent, authorize("bugReports", "read"), async (req, res, next) => {
  try {
...
router.get("/api/agent/bug-reports/mine", requireAuth, requireAgent, async (req, res, next) => {
  try {
...
router.post("/api/agent/bug-reports", requireAuth, requireAgent, async (req, res, next) => {
  try {
    const data = agentBugReportCreateSchema.parse(req.body);
...
router.put("/api/agent/bug-reports/:id", requireAuth, requireAgent, authorize("bugReports", "update"), async (req, res, next) => {
  try {
    const data = agentBugReportUpdateSchema.parse(req.body);
...
router.get("/api/agent/sites/:siteId/opportunities-stats/search", requireAuth, authorize("forms", "read"), async (req, res, next) => {
  try {
...
router.get("/api/agent/sites/:siteId/opportunities-stats/revenue", requireAuth, authorize("forms", "read"), async (req, res, next) => {
  try {
    const siteId = req.user.role === "superadmin" && req.query.site_id
      ? parseInt(req.query.site_id, 10) : req.user.site_id;
...
router.get("/api/agent/sites/:siteId/opportunities-stats/conversion", requireAuth, authorize("forms", "read"), async (req, res, next) => {
  try {
    const siteId = req.user.role === "superadmin" && req.query.site_id
      ? parseInt(req.query.site_id, 10) : req.user.site_id;
...
router.get("/api/agent/sites/:siteId/opportunities-stats/vendor", requireAuth, authorize("forms", "read"), async (req, res, next) => {
  try {
    const siteId = req.user.role === "superadmin" && req.query.site_id
      ? parseInt(req.query.site_id, 10) : req.user.site_id;
...
router.get("/api/agent/sites/:siteId/opportunities-stats/trend", requireAuth, authorize("forms", "read"), async (req, res, next) => {
  try {
    const siteId = req.user.role === "superadmin" && req.query.site_id
      ? parseInt(req.query.site_id, 10) : req.user.site_id;
...
router.put("/api/agent/sites/:siteId/opportunities/:id/owner", requireAuth, authorize("forms", "update"), async (req, res, next) => {
  try {
