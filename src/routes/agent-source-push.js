...
// ─────────────────────────────────────────────────────────────────────────
// API agent — Push bidirezionale verso il CRM sorgente.
//
...
export function registerSourcePushRoutes(router) {
  // Esegue subito il drain della coda push del sito (fire-and-forget).
  router.post("/api/agent/sites/:siteId/source-push/run", requireAgent, async (req, res, next) => {
    try {
  ...
  router.get("/api/agent/sites/:siteId/source-push", requireAgent, async (req, res, next) => {
    try {
      const siteId = parseInt(req.params.siteId, 10);
      if (!(await canAccessSite(req.user, siteId))) {
        return res.status(403).json({ error: "Accesso negato" });
