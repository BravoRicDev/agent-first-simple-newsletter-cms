...
// external_id (uuid, DEFAULT gen_random_uuid()) SIA source_id proprio, oltre a
// site_id → pattern "doppio id" classico (findByAnyId/publicId), NON il
// pattern mirror di source_workflows/funnels/custom_values.
//
// I dati arrivano dal mapper source-sync "commerce" (sezione GET /payments/
// del sorgente). ATTENZIONE nota reale: su QUESTO account sorgente l'endpoint
...
// emesso dal processor di pagamento di sorgente (stripe_url/token generati
...
router.get("/payments", async (req, res, next) => {
  try {
    ...
    let sql = `SELECT id, external_id, source_id, title, amount, currency, contact_email,
                      status, stripe_url, description, paid_at, created_at, updated_at
               FROM payment_links WHERE site_id = $1`;
    const params = [siteId];

    if (startAfterId) {
      // Cursore doppio-id: accetta UUID interno O source_id reale, site-scoped
...
// GET /payments/:paymentId — singolo payment link (UUID interno o source_id)
