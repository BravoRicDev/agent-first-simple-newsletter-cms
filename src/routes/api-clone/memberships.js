...
// Id pubblico del contatto collegato a un enrollment: source_id reale se
...
// source_id: l'unica referenza cross-risorsa sincronizzata da sorgente è il contatto.
...
router.post("/memberships/:membershipId/enroll", async (req, res, next) => {
  try {
    ...
    // Valida contatto esterno. La tabella contacts HA source_id (è sincronizzata
    // da sorgente), quindi l'automazione può passare l'id reale del contatto:
    // findByAnyId accetta sia UUID sia source_id. (memberships/courses/enrollments
    // NON hanno source_id → restano UUID-only, vedi verifica lo schema.)
