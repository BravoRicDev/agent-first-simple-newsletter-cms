...
// id/contactId: preferisce il source_id reale (parity con sorgente), ricade
// sull'UUID interno per thread/contatti mai sincronizzati da/verso sorgente —
...
async function serializeThread(row, locationId) {
  ...
  const contactId = publicId({ source_id: row.contact_source_id, external_id: row.contact_external_id });
  return {
    id,
    locationId,
    contactId,
...
// NOTA doppio-id: conversation_messages non ha una colonna source_id (schema
...
async function serializeMessage(row) {
  ...
  const convId = publicId({ source_id: row.conversation_source_id, external_id: row.conversation_external_id }) || generatedConvId;
  return {
    id: msgId,
  ...
  try {
    ...
    const results = await query(
      `SELECT c.*,
              ...
              ct.source_id AS contact_source_id,
...
router.post("/conversations/messages", async (req, res, next) => {
  try {
    ...
    const serialized = await serializeMessage({
      ...
      conversation_source_id: convRow.source_id,
...
router.get("/conversations/:conversationId/messages", async (req, res, next) => {
  try {
    ...
    if (startAfterId) {
      // Parity source_id (round 14): l'id messaggio esposto da serializeMessage
      // è source_message_id (quello reale sorgente) quando presente, quindi il
      // cursore deve accettare sia quello sia l'UUID interno.
      // conversation_messages NON ha né site_id né source_id → findByAnyId non
    ...
    const serialized = await Promise.all(
      messages.map((row) => serializeMessage({
        ...
        conversation_source_id: conv.source_id,
